...
QA_HISTORY_FILE = os.path.join(HISTORY_DIR, "qa_history.txt")
GEN_QUESTION_HISTORY_FILE = os.path.join(HISTORY_DIR, "gen_question_history.txt")
...
qs = QuestionSetter(
    question_example_file=PAST_QUESTION_FILE, main_material_file=MAIN_MATERIAL_FILE
)
        ...
        if "gen_question" in request.form:
            ...
            try:
                ...
                return render_template(
                    "index.html",
                    examples="Error occurred during processing. Please try again.",
                )
            ...
            if elected_option == "crewai":
                try:
                    ...
                    return render_template(
                        "index.html",
                        query=user_prompt,
                        answer=f"Error occurred during processing. {e} <br>Please try again.",
                    )

                # add to history
                save_history(
                    QA_HISTORY_FILE,
                    (request_time, user_prompt, elected_option, html_content),
                )
                return render_template(
                    "index.html", query=user_prompt, answer=html_content
                )
                ...
                try:
                    ...
                    return render_template(
                        "index.html",
                        query=user_prompt,
                        answer=f"Error occurred during processing. {error_message} <br>Please try again.",
                    )
                except Exception as e:
                    ...
                    return render_template(
                        "index.html",
                        query=user_prompt,
                        answer=f"Error occurred during processing. <br>Please try again.",
                    )
                save_history(
                    QA_HISTORY_FILE,
                    (request_time, user_prompt, elected_option, response.response),
                )
...
def history():
    ...
    return render_template(
        "history.html", history_gen_question=history_gen_question, history_qa=history_qa
    )
